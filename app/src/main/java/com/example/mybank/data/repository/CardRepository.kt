...
class CardRepository @Inject constructor(
    ...
    // Sync cards - DEPRECATED: Cards sync automatically via real-time Firestore listeners
    @Deprecated("Cards sync automatically via real-time Firestore listeners", ReplaceWith(""))
    suspend fun syncCards(): Resource<List<Card>> {
        return try {
            // Force local refresh - real-time listeners will update from Firestore
            Resource.Success(emptyList())
        } catch (e: Exception) {
            Resource.Error(e.message ?: "Failed to refresh cards")
        }
    }
